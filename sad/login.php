...
<h2>SIGN IN</h2>
  ...
  <div class="text-center"><br>
			<span class="small"></span><p>Not have an account? <a href="register.php" class="btn btn-outline-success">REGISTER</a></p>
		</div>
		<div class="text-center">
			<span class="small"></span> <a href="admin_login.php" class="btn btn-outline-success">ADMIN</a>
		</div>
	  ...
	  echo '<p class="text-danger" >Incorrect Credentials, Please try again!</p>';
		...
		if(mysqli_num_rows($result) == 1){
			$row = mysqli_fetch_assoc($result);
				if($row['Status'] == 1){
			echo '<script>alert ("This account is already logged in. Please create or log in another account.") ; window.location.href = "login.php"; </script>';
				exit();
	}
}
		}
